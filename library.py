...
class main:
    ...
    def code(self):
        ...
        self.photo = PhotoImage(file=r'background.png')
        self.canvas.create_image(True, True, image=self.photo, anchor=NW)
    ...
    def login(self):
        ...
        cursor = db.cursor()
        cursor.execute("SELECT * FROM UserLogin WHERE User='" + self.var1 + "' and Password='" + self.var2 + "'")
        db.commit()
        ...
        if self.ab != None:
            ...
            self.lbb.place(x=5, y=0)
            self.ig = PhotoImage(file='photo in dashboard 3.png')
            ...
            self.cur()
             ...
             tkinter.messagebox.showerror('Library System', 'Your ID or Password is invalid!')
    ...
    def cur(self):
        ...
        self.photo9 = PhotoImage(file="side canvas.png")
        self.canvas8.create_image(0, 0, image=self.photo9, anchor=NW)

        self.develop = Label(self.fm3, text='Developed By - sneha', bg='#fff', fg='#d7837f',
        ...
        self.logo = PhotoImage(file='addbook.png')
        ...
        self.log = PhotoImage(file='issuebook.png')
        ...
        self.bt4 = Button(self.fm3, text='  Return Books', fg='#fff', bg='#581845', font=('Candara', 15, 'bold'),
                          width=170, height=0, bd=7, relief='flat', cursor='hand2', command=self.returnbook,
                          activebackground='black', activeforeground='#581845')
        ...
        try:
            ...
            self.log9 = PhotoImage(file='logout.png')
    ...
    def addbook(self):
        class temp(main):
            ...
            def book(self):
                ...
                self.log = PhotoImage(file='backlogo.png')
            ...
            def submit1(self):
                try:
                    ...
                    if (self.id and self.ttl and self.aut and self.edi and self.pri):
                        ...
                        tkinter.messagebox.showinfo("Success", "Book has been added to the library succesfully")

                        # clear the entries after succesful operation
                        self.clear()
                    else:
                        tkinter.messagebox.showerror("Error", "Enter Valid Details")
    ...
    def issuebook(self):
        class test(main):
            ...
            def issue(self):
                ...
                # ERP ID of student
                self.lb = Label(self.fc, text='REG NO', bg='#ffe8ec', fg='black', font=('times new roman', 11, 'bold'))
            ...
            def check(self):
                ...
                cursor = db.cursor()
                cursor.execute("SELECT * FROM Students WHERE RegNo='" + self.b + "'")
                ...
                if self.var != None:
                    ...
                    self.lb2 = Label(self.fmii, text=self.var[1], fg='black', bg='#ffe8ec',
                    ...
                    self.lb4 = Label(self.fmii, text=self.var[2], fg='black', bg='#ffe8ec',
                                     font=('Calibri', 12, 'bold'))
                    ...
                    self.lb6 = Label(self.fmii, text=self.var[3], fg='black', bg='#ffe8ec',
                                     font=('Calibri', 12, 'bold'))
                    self.lb6.place(x=70, y=45)

                    # Contact
                    self.lb7 = Label(self.fmii, text='Contact :', fg='black', bg='#ffe8ec',
                                     font=('Calibri', 12, 'bold'))
                    self.lb7.place(x=5, y=65)
                    self.lb8 = Label(self.fmii, text=self.var[6], fg='black', bg='#ffe8ec',
                                     font=('Calibri', 12, 'bold'))
                    ...
                    # Book ID
                    self.tt = Label(self.fr, text='BOOK ID', bg='#ffe8ec', fg='#00203f',
                                    font=('times new roman', 11, 'bold'))
                    ...
                    self.bt1 = Button(self.fr, text='SUBMIT', width=8, bg='#adefd1', fg='#00203f', font=('Canara', 12,
                                                                                                         'bold'), bd=5,
                                      relief='flat', command=self.data, activeforeground='#adefd1',
                                      activebackground='#00203f')
                    self.bt1.place(x=15, y=160)

                else:
                    tkinter.messagebox.showwarning('Warning', 'This student is not registered !')
            ...
            def data(self):
                ...
                if (int(self.var[11]) >= 3):
                    try:
                        self.boot.destroy()
                        tkinter.messagebox.showerror("Unable to process request", "You exceed the limit of Books per student!")
                        self.flag = 1
                        self.cur()
                ...
                if self.value != None:
                    if (self.flag != 1):
                        ...
                        self.cal = Calendar(self.boot, selectday="day", bg='black', year=datetime.now().year, month=datetime.now().month, day=datetime.now().day)
                    ...
                    tkinter.messagebox.showerror('Book Not Found', 'No such book exists!')
            ...
            def issued(self):
                ...
                if self.n <= 3:
                    ...
                    book.execute("SELECT * FROM Students WHERE RegNo='" + self.erpid1 + "'")
                    self.issuevar = book.fetchone()
                    self.sum = self.issuevar[11] + 1
                    book.execute("UPDATE Students SET NoBook='" + str(self.sum) + "' WHERE RegNo='" + self.b + "' ")
                    db.commit()

                comm = db.cursor()
                comm.execute("UPDATE Students SET FromDate='" + str(self.x) + "', ToDate='" + str(
                    self.datecon) + "' , SubmitDate='' WHERE RegNo='" + self.b + "'")
                db.commit()

                tkinter.messagebox.showinfo('Library Management System', 'YOUR BOOK HAS BEEN ISSUED')
                self.boot.destroy()
                self.e1.delete(0, END)
            ...
            def mail(self):
                ...
                cursor = db.cursor()
                cursor.execute("SELECT * FROM Students WHERE RegNo='" + self.erpid + "'")
                self.var = cursor.fetchone()
                sender = "smartpistha002@gmail.com"
                reciever = self.var[7]
                ...
                try:
                    import smtplib
                    server = smtplib.SMTP_SSL("smartpistha002@.gmail.com", 465)
                    server.login(sender, password)
                    server.sendmail(sender, reciever, message)
                    print("mail sent")
                    tkinter.messagebox.showinfo("Library System", "Send mail Successfully !")
                except Exception as e:
                    pass
    ...
    def returnbook(self):
        class retu(main):
            def __init__(self):
                ...
                # ERP ID
                self.label8 = Label(self.f1, text='RegNo', bg='#ffe8ec', fg='black',
                                    font=('Times New Roman', 11, 'bold'))
            ...
            def retsucc(self):
                ...
                cursor1 = db.cursor()
                cursor1.execute("UPDATE Students SET FromDate='',ToDate='',Charge='" + str(
                    self.charge) + "' WHERE RegNo='" + self.entry + "'")
                db.commit()
                tkinter.messagebox.showinfo("Success", "Charges Updated and Books Returned Succesfully")
                self.tom.destroy()
            ...
            def retbook(self):
                ...
                cursor = db.cursor()
                cursor.execute("SELECT * FROM Students WHERE RegNo='" + self.entry + "'")
                db.commit()
                ...
                if self.data != None:
                    if (int(self.data[11]) >= 1):
                        ...
                        cursor.execute("UPDATE Students SET NoBook = 0, SubmitDate='" + str(
                            self.get_date) + "' WHERE RegNo='" + self.entry + "'")
                        db.commit()
                        ...
                        if self.var != None:
                            ...
                            if chm <= 0:
                                tkinter.messagebox.showinfo("Success", "Books returned successfully")
                                self.entry4.delete(0, END)

                            else:
                                self.tom = Tk()
                                self.tom.geometry("300x150+300+258")
                                self.tom.iconbitmap("logo.ico")
                                self.tom.title("Library System")
                                self.tom.resizable(0, 0)
                                self.tom.configure(bg="#ffe8ec")

                                self.lb = Label(self.tom, text="Name of Student: ", bg="#ffe8ec", fg="black",
                                                font=('Calibri', 11, 'bold'))
                                self.lb.place(x=5, y=20)
                                self.lb2 = Label(self.tom, text=self.var[1], bg="#ffe8ec", fg="black",
                                                 font=('Calibri', 11, 'bold'))
                                self.lb2.place(x=130, y=20)

                                self.charge = (5 * chm) + int(self.var[11])
                                self.lb3 = Label(self.tom, text="Fine Charge: ", bg="#ffe8ec", fg="black",
                                                 font=('Calibri', 11, 'bold'))
                                self.lb3.place(x=5, y=55)

                                self.lc2 = Label(self.tom, text=self.charge, bg="#ffe8ec", fg="black",
                                                 font=('Calibri', 11,
                                                       'bold'))
                                ...
                                self.tombtn = Button(self.tom, text='SUBMIT', background='#581845', foreground='white',
                                                     font=('Calibri', 12, 'bold'), width=8,
                                                     activebackground='black', activeforeground='#581845',
                                                     relief='flat', command=self.retsucc)
                        ...
                        tkinter.messagebox.showwarning("No Books Found", "This student does not have any book issued!")
                        self.entry4.delete(0, END)

                else:
                    tkinter.messagebox.showerror("Invalid RegNo", "This student doesn't exist!")
                    self.entry4.delete(0, END)
        ...
        class editing(main):
            ...
            def searchedit(self):
                ...
                if self.val != None:
                    ...
                    self.edcat.iconbitmap("logo.ico")
                    ...
                    tkinter.messagebox.showerror('Invalid Entry', "This Book doesn't exists!")
            ...
            def savedit(self):
                ...
                if (self.id and self.ti and self.au and self.ed and self.pi):
                    ...
                    db.commit()
                    tkinter.messagebox.showinfo('Changes Saved', 'Data has been updated successfully!')
                    self.edcat.destroy()
                    self.entry.delete(0, END)
                else:
                    tkinter.messagebox.showerror('Error', 'Enter Valid Details')
        ...
        class dele(main):
            ...
            def deldata(self):
                ...
                if (self.validation != None):
                    ...
                    db.commit()
                    tkinter.messagebox.showinfo('Succesful', 'The book is successfully removed from the store!')
                    self.entry4.delete(0, END)
                else:
                    tkinter.messagebox.showerror('Invalid Operation', 'This book does not exist!')
                    self.entry4.delete(0, END)
    ...
    def search(self):
        class demt(main):
            def delmdata(self):
                ...
                # Book ID
                self.label8 = Label(self.fc1, text='Book ID', bg='#ffe8ec', fg='black',
                                    font=('Times New Roman', 11, 'bold'))
                self.label8.place(x=85, y=65)
            ...
            def srch(self):
                ...
                if self.srval != None:
                    ...
                    self.lb2 = Label(self.top, text=self.srval[1], bg='#ffe8ec', fg='black',
                                     font=('Calibri', 12, 'bold'))
                    ...
                    self.lb4 = Label(self.top, text=self.srval[2], bg='#ffe8ec', fg='black',
                                     font=('Calibri', 12, 'bold'))
                    self.lb4.place(x=165, y=110)

                    self.lb5 = Label(self.top, text='Edition: ', bg='#ffe8ec', fg='black',
                                     font=('Calibri', 12, 'bold'))
                    self.lb5.place(x=85, y=150)
                    self.lb6 = Label(self.top, text=self.srval[3], bg='#ffe8ec', fg='black',
                                     font=('Calibri', 12, 'bold'))
                    self.lb6.place(x=165, y=150)
                    self.entryl.delete(0, END)

                else:
                    tkinter.messagebox.showwarning('Invalid Data', 'This book does not exists!')
                    self.entryl.delete(0, END)
    ...
    def mouseClick(self, event):
        ...
        #Submit Button
        self.btn1 = Button(self.rog, text='SUBMIT', fg='#606060', bg='#d6ed17', width=15, font=('Calibri', 12, 'bold'),
                        activebackground='yellow', activeforeground='#d6ed17',bd=3, relief='flat',
    ...
    def chan_pas(self):
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM UserLogin WHERE User='" + self.a + "'")
            conn.commit()
            self.data = cursor.fetchone()

            if self.data != None:
                cursor = conn.cursor()
                cursor.execute("UPDATE UserLogin SET Password='" + self.b + "' WHERE User='" + self.a + "'")
                conn.commit()
                tkinter.messagebox.showinfo("SUCCESSFUL", "Your Password is changed")
                self.rog.destroy()
            else:
                tkinter.messagebox.showerror("ERROR", "UserID doesn't exist")
                self.rog.destroy()
